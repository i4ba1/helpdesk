...
public class SNManagementController {
    ...
    @RequestMapping(value = "/overrideActivationLimit/{licenseId}", method = RequestMethod.PUT)
    public ResponseEntity<License> overrideActivationLimit(@PathVariable Long licenseId) {
        License license = snRepo.findOne(licenseId);
        if (license == null) {
            return new ResponseEntity<>(license, HttpStatus.NOT_FOUND);
        }
